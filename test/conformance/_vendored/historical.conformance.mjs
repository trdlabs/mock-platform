...
async function drainRows(t, query, label, maxPages) {
    const items = [];
    const seen = new Set();
    let cursor = null;
    let pages = 0;
    do {
        ...
        const next = p.body.nextCursor ?? null;
        if (next !== null) {
...
export async function runHistoricalConformance(t, opts = {}) {
    const skip = (check, reason) => { opts.onSkip?.({ check, reason }); };
    ...
    // rows: pagination
    const { items: acc, pages } = await drainRows(t, '/historical/rows?symbols=' + symbol + '&fromMs=0&toMs=9999999999999&limit=3', 'pagination', maxPages);
    ...
    const discoverSymbols = Array.isArray(d.body.symbols)
        ? d.body.symbols.filter((s) => typeof s === 'string' && s.length > 0)
        : [];
    const probed = [];
    for (const s of discoverSymbols.slice(0, 8)) {
        ...
        if (first !== undefined)
            probed.push({ symbol: s, firstTs: tsOf(first) });
    }
    if (probed.length < 2) {
        ...
        const { items } = await drainRows(t, `/historical/rows?symbols=${pair.join(',')}&fromMs=${windowFrom}&toMs=${windowTo}&limit=50`, 'rows multi-symbol', maxPages);
        const present = new Set(items.map((r) => r.symbol));
        if (present.size < 2) {
    ...
    for (const lim of [1, 2, 7]) {
        ...
        if (acc.length > lim) {
            ...
            assert(typeof page.body.nextCursor === 'string' && page.body.nextCursor.length > 0, `rows limit=${lim} must expose nextCursor while ${acc.length - lim} rows remain`);
        }
    }
    ...
    assert(big1.body.items.length === big2.body.items.length, `rows clamp is not deterministic: ${big1.body.items.length} then ${big2.body.items.length}`);
    ...
    const rowsResource = d.body.resources?.find((r) => r.name === 'rows');
    const maxPageItems = rowsResource?.pagination?.maxPageItems;
    assert(typeof maxPageItems === 'number' && Number.isFinite(maxPageItems) && maxPageItems > 0, `discover rows resource must declare pagination.maxPageItems, got ${JSON.stringify(maxPageItems)}`);
    ...
    assert(big1.body.items.length <= maxPageItems, `rows limit=${OVERSIZED} returned ${big1.body.items.length} rows, exceeding the declared maxPageItems ${maxPageItems}`);
    const drained = await drainRows(t, `${fullQuery}&limit=${OVERSIZED}`, 'rows oversized limit', maxPages);
    assert(drained.items.length === acc.length, `clamped pagination is lossy: ${drained.items.length} != ${acc.length}`);
